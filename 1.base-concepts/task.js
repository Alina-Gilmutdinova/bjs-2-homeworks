...
function calculateTotalMortgage(percent, contribution, amount, date) { //процент, выплата, сумма выплаты, дата 
	let totalAmount; //полная стоимость

	// код для задачи №2 писать здесь


	if (isNaN(Number(percent)) === true) {
		return (`Параметр "Процентная ставка" содержит неправильное значение "${percent}"`) //процентная ставка, введено не числовое значение 
	} else if (isNaN(Number(contribution)) === true) {
		return (`Параметр "Начальный взнос" содержит неправильное значение "${contribution}"`)
	} else if (isNaN(Number(amount)) === true) {
		return (`Параметр "Общая стоимость" содержит неправильное значение "${amount}"`)//общая сумма
	}

	let S = amount - contribution;
	let P = (percent / 12) / 100;
	let n; // количество месяцев
	let monthlyPayment; //общая выплата с учетом срока
	...
	if (date.getFullYear() < dateNow.getFullYear() || date.getFullYear() === dateNow.getFullYear() && date.getMonth() <= dateNow.getMonth()) {
		...
		n = ((date.getFullYear() - dateNow.getFullYear() - 1) * 12) + (12 - dateNow.getMonth()) + date.getMonth(); //Ежемесячная оплата
		console.log(n);
	}

	monthlyPayment = S * (P + P / (((1 + P) ** n) - 1)); //общая стоимость с учетом срока
